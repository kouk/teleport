...
func (c *TrustedClusterV2) CanChangeStateTo(t TrustedCluster) error {
	immutableFieldErr := func(name string) error {
		return trace.BadParameter("can not update %s for existing trusted cluster, delete and re-create this trusted cluster with updated %s", name, name)
	}
	...
	if c.GetEnabled() == t.GetEnabled() {
		if t.GetEnabled() {
			return trace.AlreadyExists("trusted cluster is already enabled, this update would have no effect")
		}
		return trace.AlreadyExists("trusted cluster is already disabled, this update would have no effect")
	}
